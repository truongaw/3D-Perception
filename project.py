...
def pr2_mover(object_list):
    ...
    for j in range(0,len(labels)):
        if object_name.data == labels[j]:
            ...
            pick_pose.position.y = np.asscalar(centroids[j][0])
            pick_pose.position.z = np.asscalar(centroids[j][0])
            #print pick_pose
...
if __name__ == '__main__':
    ...
    pcl_table_pub = rospy.Publisher("/table", PointCloud2, queue_size=1)
    pcl_cluster_pub = rospy.Publisher("/pcl_cluster", PointCloud2, queue_size=1)
    object_markers_pub = rospy.Publisher("/object_markers_pub", Marker, queue_size=1)
